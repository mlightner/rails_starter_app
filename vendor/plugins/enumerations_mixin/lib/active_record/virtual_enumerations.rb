...
class Module # :nodoc:
  alias_method :const_missing_without_virtual_enums, :const_missing
  def const_missing(const_id)
    const_missing_without_virtual_enums(const_id)
  rescue NameError => e
    enum_class = ActiveRecord::VirtualEnumerations.synthesize_if_defined(const_id)
    raise unless enum_class
    enum_class
  end
  alias_method :const_missing_with_virtual_enums, :const_missing
end
