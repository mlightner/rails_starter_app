  ...
  module Acts 
    module Enumerated 
      ...
      module ClassMethods  
        ...
        def all
          return @all if @all
          @all = all_unfrozen.collect{|val| val.freeze}.freeze
        end
        ...
        # Usage:
        # CustomerStatus.all_except(@customer.customer_status)
        # CustomerStatus.all_except(:pending, :active)
        # Assuming you've got methods that select specific records like all_terminated and all_suspended, 
        # you can use the returned arrays from there too!
        # CustomerStatus.all_except(CustomerStatus.all_terminated, CustomerStatus.all_suspended)
        def all_except(*args)
          all.select { |x| !x.in?(*args) }
        end
        ...
        # of the per-process rails model.  This blows up noisily just 
        # in case you try to be more clever than rails allows.
        ...
        def enforce_strict(arg)
          raise ActiveRecord::RecordNotFound, "Couldn't find #{self.name} identified by (#{arg.inspect})"
        end
      ...
      module InstanceMethods
        ...
        # The cached values could get out of synch between instances
        # and rather than completely disallow changes I make you jump 
        # through an extra hoop in case you're defining your enumeration 
